...
export function renderImoveisTemplate(data: TemplateData): string {
  const whatsappUrl = `https://wa.me/55${data.whatsapp.replace(/\D/g, "")}?text=${encodeURIComponent(`Olá! Gostaria de mais informações sobre imóveis com a ${data.companyName}.`)}`;
